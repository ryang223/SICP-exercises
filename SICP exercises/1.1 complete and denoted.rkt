#lang scheme
;;Problem: Assuming the following sequence is evaluated in order, what response will the interpreter output?
...
(if (and (> b a) (< b (* a b)))
    b
    a); it prints (4) since both predicates are true
...
;which is then used as an arguement (operand) for the procedure (operator) '+' which then prints 6
